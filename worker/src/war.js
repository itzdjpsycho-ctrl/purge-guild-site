// Extracts a full Node War result (date/location/result + every player's stats)
// from 1-4 screenshots via Claude vision. Mirror of bot/src/lib/war.js (same
// PROMPT) so the website and bot read screenshots identically. The Anthropic
// key lives in the Worker secret ANTHROPIC_API_KEY — never in the static site.
 ...
 */
export async function readWar(images, apiKey, model) {
  if (!apiKey) return { ok: false, error: "no-key" };
  if (!images || !images.length) return { ok: false, error: "No images provided." };
  try {
