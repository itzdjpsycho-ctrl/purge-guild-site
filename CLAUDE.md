...
- **Nav on all pages:** Home · War Scores · Combat Log · Roster · My Stats · VOD Review · Clips · Tachyon Tracker · Sign Ups · Dashboard (see the categorized grouping below — this is just the full page list)
- All pages share identical nav — keep in sync when adding pages
- **Sidebar nav is grouped into fixed categories** (2026-08 redesign, regrouped same month): **Overview** (Home), **Warfare** (War Scores, Combat Log, Roster, My Stats), **Media** (VOD Review, Clips), **Tools** (Tachyon Tracker), **Administration** (Sign Ups, Dashboard — placed last, below Tools). Overview and Tools are deliberately single-item categories — that's the direct result of the regroup, not an oversight. Each category header is a collapse/expand toggle — `assets/sidebar-nav.js` (included on every page right after `assets/auth.js`, no auth dependency) wires the chevrons and persists collapsed state per visitor in `localStorage["sidebarCollapsedCats"]`, same personal/browser-local pattern as Squad Roles and the Tachyon Tracker's found-state — **not** shared guild data. Nav panels use rounded corners (`border-radius`) rather than the angular clipped-corner notch — a deliberate departure from `--panel-clip` for the sidebar + `index.html` specifically; other pages' panels still use the notch and haven't been migrated. The `#authWidget` auth status/login widget sits at the bottom of the sidebar (`margin-top:auto` in its injected style, `assets/auth.js`), just above the `Purge · BDO Guild` footer text, rather than directly under the nav. **The old Guild-Master drag-to-reorder feature (`assets/nav-order.js` / `PurgeNavOrder`) was removed** when categories were introduced — category order/membership is fixed for everyone now, no per-officer customization; editing it means editing the markup on all pages (or the generator script used to do so — see git history) directly. The Worker's `GET`/`POST /nav-order` endpoints (KV key `navOrder`) are no longer called by the site and are effectively dead code, left in place rather than pulled since removing them needs a separate `wrangler deploy` the site change doesn't require.
