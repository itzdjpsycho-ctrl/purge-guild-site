...
| `worker/` | **Cloudflare Worker relay** (free tier, `workers.dev` + a KV namespace). Bridges the static site to the bot for sign-ups. Holds `DISCORD_BOT_TOKEN` as a secret and posts/edits the sheet via the Discord REST API, emitting the **same `signup:*` component customIds** the bot routes on (`src/discord.js` mirrors `bot/src/lib/embeds.js`). Endpoints: `POST /post` & `/edit` & `/op` & `/profile-op` & `/war-op` (officer-session gated — `/profile-op` also allows a player editing their own profile), `GET /state` (public, sanitized), `POST /state` & `/config` & `GET /posted` & `GET /ops` & `GET /profile-ops` & `GET /war-ops` (gated by `BOT_PUSH_SECRET`, bot-only — the bot polls these to apply profile edits and War Scores removals the website queued), and `POST /gear` (public, origin-locked + size-capped — reads AP/Awk-AP/DP off a gear screenshot via Claude vision, `ANTHROPIC_API_KEY` secret). Setup in `worker/README.md`. Deployed separately (`wrangler deploy`) — NOT served by GitHub Pages. |
