...
- **Sidebar nav is drag-reorderable by a Guild Master** (`assets/nav-order.js`, included on every page right after `assets/auth.js`). The order is global — stored in the Worker (KV key `navOrder` via `GET`/`POST /nav-order`) and applied to every visitor's sidebar, not just the Guild Master's own browser. `PurgeNavOrder.enableIfGuildMaster(state)` (called from each page's `PurgeAuth.init()` chain) turns on drag handles only when `state.role === "guildmaster"` — stricter than the general officer-tier gate used for Merge Stats/war removal/etc., since this changes navigation for everyone. Applying the saved order itself doesn't wait on auth (plain public `GET`) so the nav doesn't flash default-order-then-jump.
