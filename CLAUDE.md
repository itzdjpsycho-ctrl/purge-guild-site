...
| `worker/` | **Cloudflare Worker relay** (free tier, `workers.dev` + a KV namespace). Bridges the static site to the bot for sign-ups. Holds `DISCORD_BOT_TOKEN` as a secret and posts/edits the sheet via the Discord REST API, emitting the **same `signup:*` component customIds** the bot routes on (`src/discord.js` mirrors `bot/src/lib/embeds.js`). Endpoints: `POST /post` & `/edit` (gated by `ADMIN_POST_PASSWORD`), `GET /state` (public, sanitized), `POST /state` & `/config` & `GET /posted` (gated by `BOT_PUSH_SECRET`, bot-only). Setup in `worker/README.md`. Deployed separately (`wrangler deploy`) — NOT served by GitHub Pages. |
